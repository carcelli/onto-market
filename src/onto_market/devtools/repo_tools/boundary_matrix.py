...
def analyze(root: str | Path | None = None) -> BoundaryMatrixSnapshot:
    return BoundaryMatrixSnapshot(root=resolve_repo_root(root))


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Scaffold for architecture boundary checks")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"Boundary matrix scaffold ready at {result.root}")
    return 0
